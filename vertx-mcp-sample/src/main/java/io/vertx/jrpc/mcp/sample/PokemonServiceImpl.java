...
/**
 * Pokemon Service implementation that fetches data from PokeAPI. Provides Pokemon information with images converted to base64.
 */
public class PokemonServiceImpl implements Service {
    ...
    server.callHandler(new GetRichPokemonMethod(), request -> request.handler(requestBody -> {
      fetchRichPokemon(requestBody).whenComplete((content, err) -> {
        Content response = content != null
          ? content
          : Content.newBuilder().addContent(ContentItem.newBuilder().setText(TextContent.newBuilder().setText("Error fetching rich Pokemon").build()).build()).build();
        request.response().end(response);
      });
    }));
  ...
  private static class GetPokemonMethod implements ServiceMethod<PokemonRequest, TextContent> {
    ...
    @Override
    public GrpcMessageEncoder<TextContent> encoder() {
      return TEXT_CONTENT_ENCODER;
    }

    @Override
    public GrpcMessageDecoder<PokemonRequest> decoder() {
      return POKEMON_REQUEST_DECODER;
    }
  }
  ...
  private static class GetRichPokemonMethod implements ServiceMethod<PokemonRequest, Content> {
    ...
    @Override
    public String methodName() {
      return "GetRichPokemon";
    }

    @Override
    public GrpcMessageEncoder<Content> encoder() {
  ...
  private static class GetPokemonSpeciesMethod implements ServiceMethod<SpeciesRequest, Content> {
    ...
    @Override
    public GrpcMessageDecoder<SpeciesRequest> decoder() {
      return SPECIES_REQUEST_DECODER;
    }
  }
  ...
  private static class GetPokemonTypeMethod implements ServiceMethod<TypeRequest, Content> {
    ...
    @Override
    public GrpcMessageDecoder<TypeRequest> decoder() {
      return TYPE_REQUEST_DECODER;
    }
  }
  ...
  private CompletableFuture<TextContent> fetchPokemon(PokemonRequest request) {
    ...
    return fetchJsonFromUrl(url)
      ...
      .exceptionally(throwable -> TextContent.newBuilder()
  ...
  private CompletableFuture<Content> fetchRichPokemon(PokemonRequest request) {
    ...
    return fetchJsonFromUrl(url)
      .thenCompose(json -> {
        ...
        // This is supported by the inspector, but is not supported by the claude client
        /*contentBuilder.addContent(ContentItem.newBuilder()
          .setResourceLink(ResourceLinkContent.newBuilder()
            .setUri(url)
            .setName("Pokemon Details")
            .setDescription("Link to the Pokemon details on PokeAPI")
      ...
      .exceptionally(throwable -> Content.newBuilder()
        .addContent(ContentItem.newBuilder()
