...
public interface GrpcHttpInvoker {
  ...
  /**
   * Reports the HTTP methods this invoker binds at the path of an {@code OPTIONS} request, and the request body media
   * types accepted per method, so the server can build the {@code Allow}, {@code Accept-Post}, {@code Accept-Patch} and
   * CORS preflight headers. The invoker only contributes information: the server remains the single writer of the
   * response.
   *
